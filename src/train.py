...
df_cleaned = pre.clean_data(heart_disease_df)
...
# 결측치 처리
X_train_imputed, X_test_imputed = pre.impute_missing_values(X_train, X_test)

# 이상치 처리
X_train_clamped, train_bounds = pre.IQRClamp(X_train_imputed)
X_test_clamped = pre.IQRClamp(X_test_imputed, train_bounds)
